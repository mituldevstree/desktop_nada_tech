...
namespace NadaTech.View
{
    public partial class CheckOut : UserControl
    {
        ...
        public CheckOut(object _mainform)
        {
            InitializeComponent();
        ...
        void SaveData()
        {
            try
            {
                ...
                if (_ListTagReader.Where(w => w.IsMoveable == false && w.Status == "InStock").Count() > 0)
                {
                    NonMoveable _NonMoveable = new NonMoveable();
                    var Result = _NonMoveable.Show(NonMoveable.FormMode.Add, _ListTagReader.Where(w => w.IsMoveable == false && w.Status == "InStock").ToList());
                ...
                if (_ListTagReader.Count() > 0)
                {
                    foreach (var item in _ListTagReader)
                    {
                        ...
                        if (_AssetTagDetail.AssetStatus != "2")
                        {
                            TransactionHeader transactionHeader = new TransactionHeader();
                            transactionHeader.TransactionType = 2;
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            var Result = _AssetSelect.Show(AssetSelect.FormMode.Add, 1);
        ...
        private void GetEPC(string pc, string epc, string crc, string rssi, string antno, string per)
        {
            ...
            if (_ListTagReader.Where(w => w.TagData == epc).Count() == 0)
            {
                ...
                timer1.Stop();
                timer1.Start();
                BindDataGrid(epc, GetReaderDeviceAddrs());
            }
        ...
        private ReaderAPI.UHFReaderReportCallback uhfReaderCallbackout;
        ...
        void CIReaderConnect()
        {
            ...
            uhfReaderCallbackout = new ReaderAPI.UHFReaderReportCallback(ReaderCallbackout);
            string devinfo = "";
        ...
        private bool ConnectDev(String devinfo)
        {
            try
            {
                ...
                var iret = ReaderAPI.UHFReader_Connect(ref m_handler, devinfo, len, uhfReaderCallbackout);
        ...
        private void ReaderCallbackout(byte reader_id, UInt32 report_cmd_code, IntPtr report_data, UInt32 report_data_len)
        {
